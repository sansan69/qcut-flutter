      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: isActive ? QCutColors.emeraldBg : QCutColors.redBg,
            borderRadius: BorderRadius.circular(14),
          ),
          child: Row(children: [
            Icon(isActive ? Icons.check_circle : Icons.block, color: isActive ? QCutColors.emerald : QCutColors.red),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(_tenant.status.toUpperCase(), style: TextStyle(fontWeight: FontWeight.bold, color: isActive ? QCutColors.emerald : QCutColors.red)),
              Text(isActive ? 'Tenant is active and operational' : 'Tenant is suspended', style: TextStyle(fontSize: 12, color: isActive ? QCutColors.emerald.withValues(alpha: 0.7) : QCutColors.red.withValues(alpha: 0.7))),
            ])),
            if (isActive)
              TextButton(
                onPressed: () {
                ...
                child: const Text('Suspend', style: TextStyle(color: QCutColors.red)),
              )
            else
              TextButton(
                onPressed: () {
                ...
                child: const Text('Activate', style: TextStyle(color: QCutColors.emerald)),
              ),
          ]),
        ),
        ...
        Container(
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(color: QCutColors.purpleBg, borderRadius: BorderRadius.circular(14)),
          child: Column(children: [
            Row(children: [
              Text(plan.name, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: QCutColors.purple)),
              const Spacer(),
              Text('₹${plan.price}/mo', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: QCutColors.purple)),
            ]),
            const SizedBox(height: 12),
            Text('${plan.maxBarbers} barbers • ${plan.maxServices} services • ${plan.appointments ? "Appointments + QR" : "Token only"}',
              style: TextStyle(fontSize: 13, color: QCutColors.purple.withValues(alpha: 0.7))),
            const SizedBox(height: 16),
            Text('Change Plan', style: TextStyle(fontSize: 13, color: QCutColors.purple.withValues(alpha: 0.7))),
            ...
            Row(children: SubscriptionPlan.values.map((p) => Expanded(
              child: Padding(
                ...
                child: ChoiceChip(
                  ...
                  labelStyle: TextStyle(color: _tenant.planLevel == p.level ? Colors.white : QCutColors.charcoal),
                ),
              ),
            )).toList()),
          ]),
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        SizedBox(width: 120, child: Text(label, style: TextStyle(fontSize: 13, color: QCutColors.charcoal.withValues(alpha: 0.5)))),
        Expanded(child: Text(value, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: QCutColors.navy))),
      ]),
    );
  }
}
